...
錄影結束後，以 `verify` subcommand 或 MCP `cast_verify` 一次檢查 result status、exit code、SHA-256、byte size、event count 與 secret scan。result 的 `scan.safe_to_share` 也必須是 true。若 result 不存在、仍是 `in_progress`、digest 不符或 scan 有 finding，不要以其中的畫面或 marker 當作完成或可分享證據。
