...
TEXT_IF <畫面文字> => <text> 只在畫面含指定文字時輸入；用於輸入 `y` 本身就提交的確認頁，不會多送 Enter
...
ACTIVATE <label> WITH ENTER|SPACE 原子地定位 label 後送指定按鍵；所有選單優先使用
CHOOSE <label>      相容別名：ACTIVATE <label> WITH ENTER
TOGGLE <label>      相容別名：ACTIVATE <label> WITH SPACE
DOWN [n] / UP [n]    ⚠ 僅限無法以 label 辨識的非選單場景；n 必須為正整數
CHECKLIST_DOWN [n]   已確認在可捲動多選清單內時使用的 DOWN；strict lint 接受，但仍須以畫面或程式碼確認順序
BACKSPACE [n]        送 DEL，清除 prompt 預填值
CLEAR_LINE           相容別名；新腳本用 CTRLU 清除游標前的整行，覆寫欄位優先用 REPLACE_TEXT 系列
WAIT <ms>            ⚠ 最後手段；先考慮 EXPECT / EXPECT_QUIET
EXPECT <text>        等到畫面出現文字（預設 timeout 10s）
EXPECT@<ms> <text>   單步覆寫 timeout（慢步驟：建置、網路）
EXPECT_TRANSITION <text> 等待上一個動作後畫面真的改變，且改變後才出現 <text>
EXPECT_FRESH <text>  等到文字**第一次**出現在 viewport（自上一個 mutating step 之後）；解決 viewport scrollback 提前匹配。亦可加 @<ms> 覆寫 timeout。
EXPECT_FRESH_REGEX <pattern> 同 EXPECT_FRESH，但用 regexp。
...
FOCUS <label>        自動按 ↑/↓ 直到選單指標行含有 label，不送確認鍵
SELECT <label>       相容別名：FOCUS <label>
SNAPSHOT [label]     將渲染畫面保存到 result 的 snapshots，並傾印到 stderr（除錯用）
END_SESSION          不等待子程序，自動終止並以 aborted 保存探索錄影
QUIT                 相容別名；新腳本用 END_SESSION
```

## 必守規則

1. **選單一律優先用 `ACTIVATE <label> WITH ENTER|SPACE`；只移動指標時才用 `FOCUS <label>`。** `FOCUS` 不會送按鍵。單選/進入頁面寫 `WITH ENTER`；多選/checklist 寫 `WITH SPACE`。禁止用 `DOWN n` 猜格數。`SELECT`、`CHOOSE`、`TOGGLE` 僅為相容別名，新的 agent 腳本不得使用。label 選畫面上該行獨有的子字串。指標不是 `❯`/`>` 系列時用 `--pointer` 自訂 regexp。
1a. **Checklist 在 `WITH SPACE` 後不可自動提交。** 以畫面上的勾選符號 `ASSERT` 驗證後，才用 `ENTER_IF <角色清單的唯一提示>` 提交。目標不在目前 viewport、必須按位置移動時，改用 `CHECKLIST_DOWN n` + `SPACE`，不要用一般 `DOWN`。`CHECKLIST_DOWN` 只向 strict lint 宣告這是 checklist；n 仍須從畫面或程式碼即時算出，不可憑記憶硬編。
2. **文字欄位優先使用 `TEXT_AND_ENTER`／`REPLACE_TEXT_AND_ENTER` 原子提交。** 需要先驗證目前提示才提交時，使用一般 TEXT 後接 `ENTER_IF <目前提示的唯一文字>`；若 `y`／`n` 本身就會提交確認頁，使用 `TEXT_IF <提示> => y`，不要再補 Enter。提交/選單轉場後、下一個動作前用 `EXPECT_TRANSITION <新畫面唯一文字>`；它同時證明本次輸入已生效，避免舊畫面的文字使普通 `EXPECT` 立即通過。`EXPECT_QUIET` 只證明沒有輸出，不是畫面狀態證據。
2a. **多輪 / 重複出現的等待必須用 `EXPECT_FRESH`。** 同一字串在腳本裡出現兩次且中間有 `WAIT` 或螢幕變動時，`EXPECT` 可能匹配到上輪留下的 scrollback。`EXPECT_FRESH` 從上一個 mutating step 算起，要求該字串**新出現**。apply 階段等第二個 `PLAY RECAP`、loop 裡等下一次 prompt 刷新、wizard 多次回到同一個畫面——都該用 `EXPECT_FRESH`。strict lint 會把 `WAIT <ms>; EXPECT <generic>` 標成 error。
...
7. **秘密不落地**：密碼、token 絕不 `TEXT` 或 `REPLACE_TEXT` 進錄影。wizard 要求輸入秘密時，用 `TEXT_ENV` / `TEXT_FILE`；覆寫預填秘密用 `REPLACE_TEXT_ENV` / `REPLACE_TEXT_FILE`。載入 script 時 trec 會預先註冊其中所有 ENV／FILE 值，讓子程序最早期 output 也能遮蔽；仍必須在完成後執行 `verify`，因為 trec 不會猜測未宣告的既有秘密。以 `--secret-env <ENV>` 或 `--secret-file NAME=path` 宣告可能出現在 command、子程序 output、marker 或診斷的秘密。完整 command 預設不寫入 header；需要辨識時用 `--command-label`。只有確實需要時才開啟 `--record-command`，且仍必須搭配 `--secret-env`。
8. **長跑 apply 不得以 `EXPECT_QUIET` 判定完成。** 最後一次送出 apply 後，腳本必須以足夠上限的 `WAIT_CHILD_EXIT@<ms>` 再以 `ASSERT_EXIT 0` 判定完成。若只想要「等 apply 跑完、記錄最後畫面」的探勘錄影，腳本用 `EXPECT_FRESH@<ms> PLAY RECAP` 然後 `SNAPSHOT` + `END_SESSION`。此時 result.status 為 `ended`（不是 aborted），verify 仍會通過 integrity / cast / scan 檢查；只是 disposition = `script_ended`，必須被讀者視為「主動結束」而不是「失敗」。
...
# 設定 client-vm 這台主機
EXPECT Choose host:
ACTIVATE client-vm WITH ENTER
EXPECT_TRANSITION name>
TEXT_AND_ENTER client-vm
EXPECT_TRANSITION ip>
TEXT_AND_ENTER 192.168.122.2
EXPECT_TRANSITION configured client-vm # 這一輪的結果驗證
EXPECT Choose host:                # 確認回到主選單
ACTIVATE save & exit WITH ENTER
...
錄影結束後，以 `verify` subcommand 或 MCP `cast_verify` 一次檢查 result status、exit code、SHA-256、byte size、event count、`SESSION_END` 與 secret scan。`verify` 也會確認唯一且位於最後的 `SESSION_END` status/exit code 與 result 一致。result 會保存模式、timeout、終止原因、script SHA-256、去敏 normalized steps、最後步驟及更新時間，以及輸入指紋（inventory / vault / cwd 的 mtime + SHA-256，幫助跨環境誤判的偵測）。若 result 不存在、仍是 `in_progress`、存在未配對的 `STEP_START`、digest 不符、scan 有 finding 或 inputs.mtime 比環境當前檔案舊，**不要以其中的畫面或 marker 當作完成或可分享證據**。腳本主動 `END_SESSION` 的 result 為 `status=ended` + `termination.disposition=script_ended`；外部 signal / drive timeout 是 `aborted` / `failed` + `external_signal` / `timeout` disposition——`verify` 對這幾種終止分別處理，前者視為正常收尾。result 在長時間 apply 期間會每 30 秒 flush 一次 `last_step.heartbeat_at`、`progress.last_output_age_ms` 與 `progress.waiting_for`；poll 時若 `heartbeat_at` 超過 5 分鐘沒更新，verify 會把該次驗證標成 `progress.phase=heartbeat_stale`，這是「卡住」與「仍在跑」的可程式化區別。

## 互動模式（邊看邊按）

流程分支多、畫面不可預測、或第一次探勘時，改用互動模式：一次送一條指令，看到回覆的畫面再決定下一步。`--interactive` 可獨立使用，不需要 `--script`；若兩者同時提供，會先跑完腳本再讀取 stdin 的互動操作。

**Agent 前提：必須保留同一個 stdin session。** 以可持續寫入 stdin 的 PTY/session 啟動 `trec drive --interactive`，每次收到 `SCREEN` 回覆後，以同一個 session 送下一條操作。不要使用會在命令送出後關閉 stdin 的 heredoc、pipe 或一次性 exec。若 MCP 提供 `terminal_start`/`terminal_activate`/`terminal_key`，也可直接啟動並錄製 raw wizard：那是另一條同樣合規的 closed-loop 路徑，並非必須改寫成 `.drive` 腳本。只有沒有持續 session 和 MCP 時，才改用短 `--script` 搭配 `EXPECT`、`ASSERT` 與 `SNAPSHOT` 探測畫面，再依結果啟動下一個短腳本。
...
### 2026-07-19 — guarded confirmation, checklist navigation, MCP structured keys, and provenance warning

```text
$ go test ./... -count=1 -run '^(TestParseDriveLifecycleSteps|TestLintDriveStepsRejectsBlindTransitionsAndChecksExitPair|TestMCPTerminalWriteEnterRawModeChild|TestVerifyCastWarnsForDirtyProducerBuild)$' -v
...
=== RUN   TestMCPTerminalWriteEnterRawModeChild
=== RUN   TestMCPTerminalWriteEnterRawModeChild/plain_enter
=== RUN   TestMCPTerminalWriteEnterRawModeChild/structured_enter
=== RUN   TestMCPTerminalWriteEnterRawModeChild/paced_text_ending_in_enter
=== RUN   TestMCPTerminalWriteEnterRawModeChild/negative_delay_rejected
=== RUN   TestMCPTerminalWriteEnterRawModeChild/unknown_structured_key_rejected
--- PASS: TestMCPTerminalWriteEnterRawModeChild (0.25s)
=== RUN   TestVerifyCastWarnsForDirtyProducerBuild
--- PASS: TestVerifyCastWarnsForDirtyProducerBuild (0.03s)
PASS
ok   github.com/kjelly/trec	0.289s
```
...
$ ./trec drive --force --script <tmp>/trec-doc-guarded.drive -o <tmp>/trec-doc-guarded.cast -- sh -c "printf 'Confirm deployment'; IFS= read -r answer"
Confirm deployment
...
```text
$ go test ./... -count=1 -run '^(TestReplaceTextClearsLineAndRedactsSecretInput|TestLiteralTextAllowedOnPasswordPromptAndMarkersHideText|TestDriveWaitChildExitAndAssertExit)$' -v
warning: both GOPATH and GOROOT are the same directory (<redacted-home>/go); see https://go.dev/wiki/InstallTroubleshooting
=== RUN   TestReplaceTextClearsLineAndRedactsSecretInput
--- PASS: TestReplaceTextClearsLineAndRedactsSecretInput (0.00s)
=== RUN   TestLiteralTextAllowedOnPasswordPromptAndMarkersHideText
--- PASS: TestLiteralTextAllowedOnPasswordPromptAndMarkersHideText (0.00s)
