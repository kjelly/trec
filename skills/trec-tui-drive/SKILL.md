...
錄影結束後，讀取 `run.cast.result.json`。除了 `status`、`exit_code`、`final_screen` 與 `snapshots`，還要檢查 `cast.complete` 是 true、`cast.algorithm` 是 `sha256`，並將已關閉的 `run.cast` 的 SHA-256 與 `cast.sha256` 比對。若不符，視為不完整或被修改的錄影，不要以其中的畫面或 marker 當作執行證據。
