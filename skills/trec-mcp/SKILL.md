...
兩個啟動工具都接受 `working_directory`。錄影檔以 trec 命令的 `-o <path>` 指定；相對
路徑以 `working_directory` 為準，絕對路徑則直接寫到指定位置。每次錄影都明確傳 `-o`，
不要依賴預設的時間戳檔名。
...
`run` 與 `terminal_start` 可執行任意本機命令，僅接受使用者已授權的 command。秘密仍依 trec 的 `TEXT_ENV`、`TEXT_FILE`、`--secret-env`、`--secret-file` 規則處理；不要把秘密放在 MCP tool arguments 或 command argv。
