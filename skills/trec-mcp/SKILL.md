...
每次由 `record` 或 `drive` 產生的 cast 都有同名 `.result.json`。錄影程序結束後，先讀取該摘要的 `status`、`exit_code`、`error`、`final_screen` 與 `snapshots`；不要只因 MCP process 已停止就宣告工作成功。再檢查 `cast.complete=true`、`cast.algorithm="sha256"`，並重算已關閉 `.cast` 的 SHA-256 比對 `cast.sha256`。digest 不符、檔案大小不符 `cast.byte_size`，或摘要不存在時，視為不完整或被修改的錄影。使用 `record` 時，trec 會傳遞子程序的非零 exit code。
