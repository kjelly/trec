...
每次由 `record` 或 `drive` 產生的 cast 都有同名 `.result.json`。錄影程序結束後，先讀取該摘要的 `status`、`exit_code`、`error` 與 `final_screen`；不要只因 MCP process 已停止就宣告工作成功。使用 `record` 時，trec 會傳遞子程序的非零 exit code。
